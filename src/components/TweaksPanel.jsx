...
export const applyTweaks = (t) => {
  ...
  root.style.setProperty('--r-md', t.density === 'compact' ? '8px' : '10px');
...
export const TweaksPanel = ({ tweaks, setTweaks, onClose }) => {
  ...
  return (
    <div style={{ position: 'fixed', bottom: 20, right: 20, zIndex: 100, width: 280, background: 'var(--surface)', border: '1px solid var(--border-strong)', borderRadius: 'var(--r-lg)', boxShadow: 'var(--shadow-pop)', overflow: 'hidden', animation: 'fadeIn 0.2s ease' }}>
      ...
      <div style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {accentOptions.map(a => (
              <button key={a.id} onClick={() => update('accentColor', a.id)} title={a.label}
                style={{ width: 28, height: 28, borderRadius: '50%', background: `oklch(0.78 0.17 ${a.hue})`, border: `2px solid ${tweaks.accentColor === a.id ? 'var(--text)' : 'transparent'}`, cursor: 'pointer' }}
              />
            ))}
          </div>
        </div>
        <div>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            {[{ id: 'cool', label: '차가운' }, { id: 'neutral', label: '뉴트럴' }, { id: 'warm', label: '따뜻한' }].map(t => (
              <button key={t.id} className={'btn sm ' + (tweaks.bgTone === t.id ? 'primary' : '')} onClick={() => update('bgTone', t.id)} style={{ flex: 1 }}>{t.label}</button>
            ))}
          </div>
        </div>
        <div>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            {[{ id: 'comfortable', label: '편안' }, { id: 'compact', label: '컴팩트' }].map(d => (
              <button key={d.id} className={'btn sm ' + (tweaks.density === d.id ? 'primary' : '')} onClick={() => update('density', d.id)} style={{ flex: 1 }}>{d.label}</button>
            ))}
          </div>
        </div>
        <div style={{ fontSize: 11, color: 'var(--text-4)', textAlign: 'center', marginTop: 2 }}>변경사항은 자동 저장됩니다</div>
      </div>
    </div>
  );
};
